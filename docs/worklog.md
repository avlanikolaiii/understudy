...
- what: `ReportEngine.report(csv:week:client:agency:)` returns a Sendable `WeeklyReport` with Markdown, source-data completeness, missing cells, and typed unrounded figures for both weeks and their changes. Foundation only; deterministic dates/table; explicit AI placeholders; no file or network access. API and input rules are in `docs/app/report-engine.md`.
